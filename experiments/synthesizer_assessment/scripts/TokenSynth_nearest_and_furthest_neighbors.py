...
def compute_nearest_neighbor_each_trait():
    """
    Compute and save the nearest neighbor audio samples for each instrument, condition type, and timbre trait.

    This function:
    1. For each condition type, loads the predictions and ground truth data.
    2. For each instrument and timbre trait, finds the synthesized sample closest to the ground truth.
    3. Saves the nearest neighbor audio and its spectrogram.

    Steps:
    - For each condition type, load the predictions and ground truth.
    - For each instrument and timbre trait, compute the Euclidean distance between each synthesized sample and the ground truth.
    - Save the nearest neighbor audio and its spectrogram to disk.
...
def compute_furthest_neighbor_each_trait():
    """
    Compute and save the furthest neighbor audio samples for each instrument, condition type, and timbre trait.

    This function:
    1. For each condition type, loads the predictions and ground truth data.
    2. For each instrument and timbre trait, finds the synthesized sample furthest from the ground truth.
    3. Saves the furthest neighbor audio and its spectrogram.
